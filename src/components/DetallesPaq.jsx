...
import { ArrowLeft, Gem, CheckCircle2, Smile, Star, Info } from 'lucide-react';
import WhatsAppBtn from './WhatsAppBtn';
import { PACKAGES } from '../data/DataPaq';
import '../styles/DetallesPaq.css';

const DetallesPaq = ({ pkgId, navigate }) => {
  const pkg = PACKAGES.find(p => p.id === pkgId);
  if (!pkg) return <div>Paquete no encontrado</div>;
  const wpText = `¡Hola MyM! Quiero consultar presupuesto para el paquete *${pkg.name}* (${pkg.size}).`;
    ...
    <div className="detalles-container">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="detalles-card">
        <div className="detalles-header">
          <div className="detalles-header-icon"><Gem size={40} /></div>
        </div>
        ...
        <div className="detalles-content">
          <div className="detalles-top-section">
            ...
            <div className="detalles-quote-box">
              ...
              <WhatsAppBtn label="Pedir cotización" text={wpText} className="w-full py-3.5 px-8" />
